document.addEventListener('DOMContentLoaded', () => {
  ...
  //스크롤시 header 색상 바꾸기, blur 바꾸기 + 다크 오버레이
  ...
  window.addEventListener('scroll', () => {
    ...
    const opacity = Math.min(st / vh * 0.25, 0.25);
    $overlay.style.opacity = opacity;
  });

  // SNS 오프셋 CSS 변수 설정 (css animation용)
  function setSnsOffset() {
    const $snsShowcase = document.querySelector('.sns-showcase');
    if (!$snsShowcase) return;
    const offset = ($snsShowcase.offsetWidth / 2) - 210;
    document.documentElement.style.setProperty('--sns-offset', `${offset}px`);
  }
  setSnsOffset();
  window.addEventListener('resize', setSnsOffset);
